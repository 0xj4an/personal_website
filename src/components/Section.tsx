...
import { Html, MeshDistortMaterial, shaderMaterial } from '@react-three/drei';
import * as THREE from 'three';
import { Section as SectionType } from '../app/page';
import React from 'react';

// Custom planet shader with animated FBM noise + fresnel rim
const PlanetShaderMaterial = shaderMaterial(
  ...
  `
    ...
    // Simplex-style noise functions for FBM
    ...
    void main() {
      ...
      vec3 noiseCoord = vec3(vUv * 4.0, uTime * 0.15);
      float noise = fbm(noiseCoord);

      // Base planet color with noise variation
      vec3 surfaceColor = uColor * (0.6 + noise * 0.5);

      // Brighter bands
      float bands = sin(vUv.y * 20.0 + noise * 3.0 + uTime * 0.1) * 0.1;
      surfaceColor += bands;

      // Fresnel rim glow (atmosphere edge effect)
      vec3 viewDir = normalize(-vPosition);
      float fresnel = pow(1.0 - max(dot(viewDir, vNormal), 0.0), 3.0);
      vec3 rimColor = uColor * 2.0;
      surfaceColor += rimColor * fresnel * (0.5 + uHovered * 0.5);
...
// Type declarations for JSX - using 'any' for custom shader materials
// since R3F JSX element types need ref/attach/etc. that raw THREE types don't include
...
const SectionComponent = forwardRef<THREE.Mesh, SectionProps>(
  ({
    ...
    const atmosphereRef = useRef<THREE.ShaderMaterial & { uIntensity: number }>(null);
    const ringRef = useRef<THREE.Mesh>(null);
    const [hovered, setHovered] = useState(false);
    const isCenter = id === 'home';
    const hasRing = id === 'studies' || id === 'experience';
    ...
    useFrame((state) => {
      ...
      if (atmosphereRef.current) {
        const pulse = Math.sin(state.clock.elapsedTime * 1.5 + position[0]) * 0.2 + 0.8;
        atmosphereRef.current.uIntensity = isModalOpen ? 0.05 : (hovered ? 1.2 : pulse * 0.6);
      }
      if (ringRef.current) {
    ...
    return (
      <group position={position}>
        ...
        {/* Main planet - custom GLSL shader for non-center, MeshDistortMaterial for center */}
        <mesh
          ref={(node) => {
        ...
        {/* Second atmosphere layer - closer, more intense */}
        <mesh scale={scale * 1.08}>
          <sphereGeometry args={[1, 32, 32]} />
          <meshBasicMaterial
            color={color}
            transparent
            opacity={isModalOpen ? 0.01 : (hovered ? 0.12 : 0.04)}
            depthWrite={false}
            blending={THREE.AdditiveBlending}
          />
        </mesh>
        ...
        {/* Ring */}
        {hasRing && (
          <mesh ref={ringRef} scale={scale} rotation={[Math.PI / 2.5, 0, 0]}>
        ...
        {/* Orbiting moons for home planet */}
...
function OrbitingMoon({ color, parentScale, speed, distance, size }: {
  ...
  useFrame((state) => {
    if (moonRef.current) {
      ...
      moonRef.current.position.z = Math.sin(t) * distance * parentScale;
      moonRef.current.position.y = Math.sin(t * 0.5) * 0.5;
    }
  });
